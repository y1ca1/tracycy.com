...
_{frontmatter.publishedAt}_ • _{frontmatter.readingTime} min read_
...
- A hybrid data structure for blocks
  ...
  - A set of segregated free lists, with _mini-block_ enabled
...
- Each of the segregated lists of normal free blocks is a _circular doubly linked list_, while for the mini free blocks there is a _circular singly linked list_.
...
- Checking the heap (implicit list, segregated list):
  - Check for _epilogue_ and _prologue_ blocks.
  - Check each block’s _address alignment_.
  - Check blocks lie within _heap boundaries_.
  - Check each block’s _header_ and _footer_: size (minimum size), previous/next, allocate/free bit consistency, _header_ and _footer_ matching each other.
  - Check _coalescing_: no consecutive free blocks in the heap.
